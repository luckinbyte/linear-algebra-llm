...
**把奇异值画出来：单位圆→椭圆，半轴就是奇异值。** 特征向量要求矩阵是方阵，但奇异值对**任意**矩阵都成立，画法一样：把输入空间的单位圆经过矩阵 $A$，在输出空间得到一个椭圆。这个椭圆的每一条半轴——

- 半轴的**长度** = 奇异值 $\sigma_i$（按从大到小排，最长的半轴 = $\sigma_1$）；
- 半轴在**输入空间**里的方向 = 右奇异向量 $\mathbf{v}_i$（$V^T$ 的行）；
- 半轴在**输出空间**里的方向 = 左奇异向量 $\mathbf{u}_i$（$U$ 的列）。

所以奇异值的含义一句话就清楚了：**它就是矩阵沿各个正交方向"拉伸量"的排序**，$\sigma_1$ 是最大的拉伸。当 $A$ 是对称正定矩阵时，SVD 退化成 EVD，半轴方向恰好就是特征向量、半轴长度就是特征值——上一章和这一章在此合流。
